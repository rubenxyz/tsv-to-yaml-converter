...
class Config(BaseModel):
    ...
    yaml_indent: int = Field(
        default=2, ge=1, le=8, description="YAML indentation level"
    )
    yaml_width: int = Field(default=120, ge=80, le=200, description="YAML line width")
    mappings_file: Optional[str] = Field(
        default="mappings.json", description="Path to field mappings configuration file"
    )
    ...
    def load_mappings(
        self, mappings_path: Optional[Path] = None
    ...
    def save_mappings(
        self, mappings: Dict[str, Dict[str, str]], mappings_path: Optional[Path] = None
    ) -> None:
