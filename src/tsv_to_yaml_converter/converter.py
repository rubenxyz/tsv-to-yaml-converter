...
class TSVToYAMLConverter:
    ...
    def __init__(self, project_root: Path, config: Optional[Config] = None):
        ...
        self.yaml_writer = YAMLWriter(
            yaml_indent=self.config.yaml_indent,
            yaml_width=self.config.yaml_width
        )
    ...
    def convert_tsv_to_yaml(self, tsv_file: Path, output_file: Path, project_title: Optional[str] = None) -> bool:
        ...
        try:
            ...
            project_title = self.data_processor.get_project_title(tsv_file, project_title)
            
            # Initialize project structure
            project = self.data_processor.initialize_project(project_title, len(df))
            
            # Process data into hierarchical structure
            epochs_dict = self.data_processor.process_tsv_data(df)
            
            # Convert to Pydantic models
            project = self.data_processor.build_project_structure(project, epochs_dict)
